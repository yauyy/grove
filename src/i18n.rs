...
fn en() -> HashMap<&'static str, &'static str> {
    ...
    m.insert(
        "no_projects",
        "No projects registered. Use 'grove add <path>' to register a project.",
    );

    // grove remove
    m.insert("select_project_remove", "Select project to remove");
    m.insert(
        "project_in_workspaces",
    ...
    m.insert(
        ...
        "No projects registered. Use 'grove add <path>' first.",
    );
    ...
    m.insert(
        "delete_with_changes",
        "There are uncommitted changes. Delete anyway?",
    );
    ...
    m.insert(
        ...
        "Workspace '{}' branch renamed to '{}'",
    );

    // grove delete (branch prompt)
    m.insert(
        "delete_local_branch",
        "Delete local branch '{}'? (remote branches will not be affected)",
    );

    // grove status
    m.insert(
        "no_workspaces_status",
        "No workspaces. Create one with 'grove create'.",
    );
    ...
    m.insert(
        "no_workspaces_found",
        "No workspaces found. Create one with 'grove create'.",
    );
    ...
    m.insert(
        "language_invalid",
        "Invalid language: '{}'. Valid options: en, zh",
    );
...
fn zh() -> HashMap<&'static str, &'static str> {
    ...
    m.insert(
        "delete_local_branch",
        "是否同时删除本地分支 '{}'？（该操作不会删除远程分支）",
    );

    // grove status
    m.insert(
        "no_workspaces_status",
        "暂无工作区。使用 'grove create' 创建。",
    );
    m.insert("clean", "干净");
